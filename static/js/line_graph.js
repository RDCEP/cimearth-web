function CimEarthLineGraph() {
  ...
  var min_d, max_d, min_domain, max_domain;
  var x = d3.scale.linear().domain([start_year, end_year]).range([0, width]),
  ...
  var svg = d3.select("#graph").append("svg")
    ...
    line = d3.svg.line()
      ...
      .y(function(d,i) { return y(d-offset); }),
  ...
  this.redraw = function(dt, r1, r2, c1, c2) {
    d3.json('http://cimearth.obstructures.org/json/'+dt+'/'+r1+'/'+r2+'/'+c1, function(error, data) {
  ...
  this.draw = function() {
    d3.json('http://cimearth.obstructures.org/json/price/USA/MEX/OIL/', function(error, data) {
      ...
      dots_layers = regions
        ...
        .attr('class', function(d, i) { return d.region; });
      dots = dots_layers.selectAll('.dot')
        ...
        .on('mouseover', function(d, i) {
          var tdots = d3.selectAll('.dot-'+i)
            ...
            .each(function(dd, ii) {
              tooltip
                .html(tooltip.html() + '<b style="color:' + color(dd.color) + '">'+ dd.region + '</b>: '+ dd.data + '<br>');
            });
            tooltip
